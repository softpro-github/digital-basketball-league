...
@section('page-title','Edit League')
@section('content')
<div class="card p-4" style="max-width:600px">
    <h5 class="fw-bold mb-4">Edit: {{ $league->name }}</h5>
    @if($errors->any())
    ...
    <form method="POST" action="{{ route('admin.leagues.update', $league) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="season" class="form-control" value="{{ old('season', $league->season) }}" required>
        </div>
        <div class="mb-3">
            <label class="form-label">Status *</label>
            <select name="status" class="form-select" required>
                @foreach(['upcoming','active','completed'] as $s)
                <option value="{{ $s }}" {{ old('status', $league->status) === $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                @endforeach
            </select>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="date" name="start_date" class="form-control" value="{{ old('start_date', $league->start_date?->format('Y-m-d')) }}">
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label">End Date</label>
                <input type="date" name="end_date" class="form-control" value="{{ old('end_date', $league->end_date?->format('Y-m-d')) }}">
            </div>
        </div>
        <div class="mb-4">
